...
const Reply = ({ index, id }) => {
  ...
  const replyMode = useSelector(state =>
    state.root.post.get('posts').get(index).get('replyMode')
  );
  const replyBody = useSelector(state =>
    state.root.post.get('posts').get(index).get('replyBody')
  );
      ...
      {replyMode &&
        <div className={styles.u_full_width}>
          <form>
            ...
            ></textarea>
            <div>
              [<span className={styles.postReplyBotton} onClick={handleSubmitReply} onKeyDown={handleKeyDown} tabIndex={0} role="button"> reply </span>]
            </div>
            <div>
              [<span className={styles.cancelReplyButton} onClick={handleToggleReplyMode} onKeyDown={handleCancelKeyDown} tabIndex={0} role="button"> cancel </span>]
