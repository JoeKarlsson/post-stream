...
class ErrorLogger {
  ...
  private maxLogs: number = 100; // Keep only last 100 errors in memory
   ...
   */
  storeInLocalStorage(errorEntry: unknown): void {
    try {
   ...
   */
  async sendToExternalService(errorEntry: unknown): Promise<void> {
    try {
   ...
   * Get recent errors
   */
  getRecentErrors(limit: number = 10): unknown[] {
